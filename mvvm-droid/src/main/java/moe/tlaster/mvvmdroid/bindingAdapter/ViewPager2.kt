...
import moe.tlaster.mvvmdroid.BR
...
fun <T> items(viewPager2: ViewPager2, list: List<T>, layoutId: Int) {
    ...
    val id = Class.forName("${viewPager2.context.applicationContext.packageName}.BR").getField("model").get(null) as Int
    viewPager2.adapter = DataBindingAdapter<T>(ItemSelector(layoutId, id)).also { adapter ->
        adapter.items = list
        onClick?.let {
            adapter.onClick = it
        }
